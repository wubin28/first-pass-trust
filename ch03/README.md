...
> 请阅读本书第二章的内容，特别留意其中的端到端测试。我打算在第三章用 EARS + TDD 开发一个新需求 `Strict Header Schema Validation Mode`，它的描述如下：[粘贴上面的需求描述、三条规则和五条"明确不做"]。
>
> 请回答：这些新需求该如何添加到第二章那个"读取 Excel 导出的 CSV"的端到端测试中，以便我将来进行测试？并能方便 commons-csv 的使用者在使用该框架前就了解这些规则？是把这些规则写到文档里给用户看，还是有其他更方便的途径让用户知道？
>
> 请提供 3 个解决方案及其优劣势供我选择，最后推荐其中一个，并说明理由。
...
>
> 然后为**每个** user story 生成下列内容：
> 1. **英文标题**（要求揭示用户价值）；
> 2. **"as a ..., I want to ..., so that ..." 格式的英文 user story 正文**；
> 3. **中英对照的 EARS** [参考：EARS 的定义与五种句型]；
> 4. **决策表** [参考：决策表测试的定义]；
> 5. **中英对照的 happy path 和 sad path 验收标准** [参考：验收标准的定义]；
> 6. **两段中英对照的提示词**，分别用 `superpowers:test-driven-development` 和 `superpowers:requesting-code-review` 这两个 skill 来实现该 user story 并完成代码评审，以便我复制粘贴给 Codex。要求：
>    - TDD 提示词中必须要求遵循上述 EARS、决策表、验收标准；
>    - 并确保 [源码目录] 下的所有自动化测试在新需求实现前和实现后都运行成功。
