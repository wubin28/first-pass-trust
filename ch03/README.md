...
### 3.7.1 请求评审：喂给 Codex 的真实提示词

3.4 节其实已经为每个故事各生成了一段 `requesting-code-review` 提示词。但四个故事现在已经全部实现完了，四段分开的评审提示词得合并成一段——而且评审的重点要变。下面这段，是实际喂给 Codex、让它调用 `superpowers:requesting-code-review` 的完整提示词：
...
> 为每一条决策表规则和验收标准建立"需求—证据"矩阵，每行写出直接覆盖它的自动化测试类名/方法名，另行写出每个可运行手工 e2e 入口、fixture、参数和预期结果，并分类为：(1) 自动化直接覆盖、(2) 仅手工 e2e、(3) 仅间接、(4) 未覆盖；手工 e2e 不等于自动化覆盖，对类别 2–4 给出最小整改方案。

这四档判定是这一节最重要的产出。特别是第 (2) 档——**"仅手工 e2e"**。
...
提示词里还加了一条硬判据，直接从 spec 里翻出来：**"若实现把'只要求声明列相对顺序'错误地做成'必须相邻'，应判定为不合格。"**（对应 3.1.4 节那个陷阱。）而"手工 e2e 不等于自动化覆盖"这句判据，则直接封死了一条自我安慰的退路——不能拿"反正跑过手工脚本"来搪塞覆盖率。
...
**人在这一步做了一个明确的范围决策**：不是把评审报告里的 Critical、Important、Minor 全部照单修完，而是圈定一个更小的必修范围。实际喂给 Codex 的提示词是这样的：
...
这段提示词里有两处设计值得停一下。第一，**"把评审报告当权威范围清单，只修列出项，不扩范围"**——报告里的 Important 有 4 条、Minor 有 3 条，这次都不碰，必修范围只锁定一条 Critical、五条"仅手工 e2e"验收标准、一行"未覆盖"。**评审报告不是待办清单，是信息；范围是人定的。**

第二，**"全量失败则先按规格分析、最小修正，绝不为凑绿削弱正确断言"**——这是这段提示词的灵魂，也是 `receiving-code-review` 这个 skill 的精神所在。这句话防的是 AI 最擅长的一种堕落：测试红了，把断言改松，绿了。
...
问题问出来了，直接把下面这段提示词喂给 Codex——保留了实际的分支名和 commit range（它们决定了"新增测试"的准确范围），本机路径已换成占位符：
...
这段提示词的分量，在于它认清了一件事：**你正在授权一个 AI 故意破坏你的生产代码，然后指望它自己收拾干净。** 一个脏的工作树 + 一个"顺手清理一下"的 AI = 你没提交的改动没了。所以第二段"脏树立即停止"、禁用破坏性命令、"每次注入后用反向补丁精确恢复并验证"这三条，一条都不能少。

第五段对"什么算成功的故障注入"的定义，同样关键：**编译失败、测试发现失败、超时、无关的失败，都不算成功的故障注入。** 出现意外行为时，遵循 `superpowers:systematic-debugging` 诊断、恢复工作树，把该用例报告为不确定或受阻，而不是宣称覆盖成功。

第四段对"该注入什么故障"的约束同样明确：**选一个应该被该测试的目标断言捕获的缺陷**——绕过 schema 挂载、省略某条必填表头校验分支、放行未知表头、关闭声明表头顺序比较、把校验挪到记录暴露之后、从异常里删掉缺失/未知/违规表头的细节。**不要注入与被测行为无关的表面故障，也不要改测试本身**——后一条防的是最愚蠢也最容易发生的作弊：把测试改坏，它当然红。
