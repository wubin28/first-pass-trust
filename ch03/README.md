...
这段提示词的分量，在于它认清了一件事：**你正在授权一个 AI 故意破坏你的生产代码，然后指望它自己收拾干净。** 一个脏的工作树 + 一个"顺手清理一下"的 AI = 你没提交的改动没了。所以第一段禁用破坏性命令、第二段"脏树立即停止"和"每次注入后用反向补丁精确恢复并验证"这三条，一条都不能少。
...
第四段对"该注入什么故障"的约束同样明确：**选一个应该被该测试的目标断言捕获的缺陷**——绕过 schema 挂载、省略某条必填表头校验分支、放行未知表头、关闭声明表头顺序比较、把校验挪到记录暴露之后、从异常里删掉缺失/未知/违规表头的细节。**不要注入与被测行为无关的表面故障。**
